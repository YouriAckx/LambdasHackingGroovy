 ...
 *
 * Also seen at http://nighthacking.com/events/lambdas-hacking-with-brian-goetz
 * Source at  https://github.com/NightHacking/LambdasHacking
 *
 * This is a translation of the lab solutions in Groovy 2.2.1
 *
 * IntelliJ users:
 * because of http://youtrack.jetbrains.com/issue/IDEA-103435,
 * the @Grapes annotation will cause
 * java.lang.NoClassDefFoundError: Could not initialize class exercises.ExerciseAnswers.
 * To work around this issue, run the tests, disregard error/msg,
 * and comment out the @Grapes annotation. jUnit lib will have been downloaded.
 * Now you can run the tests.
 */

@Grapes(
    @Grab(group='junit', module='junit', version='4.9')
)
class ExerciseAnswers {
    ...
    @Test
    void upperCaseWords() {
        List<String> output = wordList*.toUpperCase()
    ...
    void findEvenLengthWords() {
        List<String> output = wordList.findAll({it.length() % 2 == 0})
    ...
    public void countLinesInFile() throws IOException {
        long count = reader.iterator().count{it}
    ...
    public void joinLineRange() throws IOException {
        def output = reader.iterator().drop(2).take(2).join()
    ...
    public void lengthOfLongestLine() throws IOException {
        int longest = reader.iterator()*.length().max()
        /*
        int longest =
            reader.lines()
    ...
    public void listOfAllWords() throws IOException {
        List<String> output = reader
                .iterator()
                .collect{line -> line.split(REGEXP)}
                .flatten()
                .findAll{word -> word.length()}
        /*
        List<String> output =
            reader.lines()
    ...
    public void sortedLowerCaseDistinctByLengthThenAlphabetically() throws IOException {
        List<String> output = reader
                ...
                .findAll{word -> !word.isEmpty()}
                .unique()
                .sort{w1, w2 -> w1.length() <=> w2.length() ?: w1 <=> w2}
    ...
    public void wordFrequencies() throws IOException {
        Map<String, Long> map = reader
                ...
                .flatten()
                .findAll { !it.isEmpty() }
                .countBy { it }         // http://mrhaki.blogspot.be/2011/04/groovy-goodness-group-and-count.html
